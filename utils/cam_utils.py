...
def crf_with_alpha(orig_img, norm_cam, valid_classes, alpha, mask_guided=False):
    # Get foreground score map
    fg_score = np.take(norm_cam, valid_classes, axis=0)
    ...
    crf_cam = np.zeros_like(norm_cam)
...
def eval_cam(args):
    ...
    #                     }
    pred_types = dict()
    for key in pred_dict['2007_003525'].keys():
    ...
    with open(os.path.join(args.result_dir, 'detail_performance.json'), 'w') as json_file:
        json.dump(performance_dict, json_file, indent=4)

    with open(os.path.join(args.result_dir, 'miou_performance.json'), 'w') as json_file:
...
def crf_inference(img, probs, t=10, scale_factor=1, num_labels=21, sigma_xy=80, sigma_rgb=13):
    ...
    d.addPairwiseBilateral(sxy=sigma_xy/scale_factor, srgb=sigma_rgb, rgbim=np.copy(img_c), compat=10)
    Q = d.inference(t)
...
def crf_inference_inf(img, probs, t=10, scale_factor=1, num_labels=21, sigma_xy=83, sigma_rgb=5):
    ...
    d.setUnaryEnergy(unary)
    d.addPairwiseGaussian(sxy=3/scale_factor, compat=3)
    d.addPairwiseBilateral(sxy=sigma_xy/scale_factor, srgb=sigma_rgb, rgbim=np.copy(img_c), compat=4)
    Q = d.inference(t)
