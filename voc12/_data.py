...
class VOC2012ClsDepthDataset(Dataset):
    def __init__(self, image_name_list_path, voc12_root, transform=None, transform2=None):
    ...
    def __getitem__(self, idx):
        ...
        # inverse_depth, _ = utils.read_pfm(get_depth_path(name, self.voc12_root)[0])
        # th = 10.0
        ...
        inverse_depth = cv2.imread(get_depth_path(name, self.voc12_root)[1])
        ...
        if self.transform2:
            croppings = None
            img, orig_img, croppings, inverse_depth = self.transform2([img, orig_img, croppings, inverse_depth])
        else:
            croppings = np.ones_like(orig_img)[:, :, 0]

        img = torch.from_numpy(img.copy())
        orig_img = torch.from_numpy(orig_img.copy())
        croppings = torch.from_numpy(croppings.copy())
        inverse_depth = torch.from_numpy(inverse_depth.copy())
        label = torch.from_numpy(self.label_list[idx])

        return name, img, label, orig_img, croppings, inverse_depth
...
class VOC12ClsDatasetMSF(VOC12ClsDatasetVAL):
